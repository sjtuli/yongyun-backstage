<template>
	<div class="tab" ref="tabChange">
		<div class="dflex h100">
			<div class="tab_content dflex_c h100 margin_r_10 crpr" v-for="(item, i) in tagsList" :class="{ active: isActive(item.path) }" :key="i">
	...
	methods: {
		...
		setTags(route) {
			...
			if (!isExist) {
				...
				this.tagsList[this.tagsList.findIndex(x => x.path === curPath || x.title == route.query.tab)].path = curPath;
			}
